...
utsattBrott <- df %>% 
  ...
  mutate(Utsatt1 = case_when(Utsatt > 0 ~ "Utsatt för brott")) %>% 
...
ggplot(df.kolada,aes(x = År, y = Andel, group = Kommun, color = Kommun)) +
  geom_line(alpha = 0.5, linewidth = 0.5, linetype = 3) +
...
ggplot(df.kolada_Simon,aes(x = År, y = Andel, group = Kommun, color = Kommun)) +
  geom_line(alpha = 0.5, 
            linewidth = 0.5, linetype = 3) +
  geom_point(alpha = 0.5, 
             size = 1.2) +
  geom_line(data = filter(df.kolada_Simon, Kommun == fokusKommun), alpha = 1) +
  geom_point(data = filter(df.kolada_Simon, Kommun == fokusKommun), alpha = 1) +
  scale_x_continuous(guide = guide_axis(n.dodge = 2)) +
  scale_y_continuous(limits = c(0, 100)) +
  scale_color_brewer(type = "qual", palette = "Dark2") +
  theme_minimal() +
...
df.kolada_Simon <- KOLADAs %>% 
  filter(Kön == "Alla") %>% 
  filter(Kommun %in% jmfKommun) %>% 
  filter(KPI %in% c("Gymnasieelever med indraget studiestöd pga. ogiltig frånvaro, hemkommun, andel (%)",
...
ggplot(df.koladaS, aes(x = År, y = Andel, group = Kommun, color = Kommun)) +
  geom_line(
    alpha = 0.5,
    linewidth = 0.5,
    linetype = 3
  ...
  geom_line(data = filter(df.koladaS, Kommun == fokusKommun), alpha = 1) +
  geom_point(data = filter(df.koladaS, Kommun == fokusKommun), alpha = 1) +
  scale_x_continuous(guide = guide_axis(n.dodge = 2)) +
  scale_y_continuous(limits = c(0, 100)) +
  scale_color_brewer(type = "qual", palette = "Dark2") +
  theme_minimal() +
...
Utifrån tidigare efterfrågan på mera exakta siffror är denna med som exempel på vad som kan tas fram.
```{r}
### Tabell psykiska/psykosomatiska besvär åk 9
